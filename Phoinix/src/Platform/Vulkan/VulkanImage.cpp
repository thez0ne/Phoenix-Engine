   ...
   void VulkanImage::SetData(const void* data)
   {
      ...
      if (!m_StagingBuffer)
      {
         // Create the Upload Buffer
         {
            ...
            VKASSERT(vkAllocateMemory(
                        VulkanDevice::Device(), &allocInfo, nullptr, &m_StagingBufferMemory),
                     "Failed to allocate image staging buffer");
            VKASSERT(vkBindBufferMemory(
                        VulkanDevice::Device(), m_StagingBuffer, m_StagingBufferMemory, 0),
